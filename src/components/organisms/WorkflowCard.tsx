...
export function WorkflowCard({
  ...
  const handleAddStep = () => {
    if (!newStepName.trim()) {
      toast({
        variant: "destructive",
        title: "Nama langkah tidak boleh kosong.",
      });
      return;
    }
    ...
    router.push(`${newStep.href}&edit=true`);
  };
  ...
  const handleRemoveStep = (stepId: string) => {
    ...
    onUpdate({ ...workflow, steps: updatedSteps });
    toast({
      title: "Langkah Dihapus",
      description: "Langkah alur kerja telah dihapus.",
    });
  };

  const handleEditStep = (href: string) => {
    router.push(`${href}&edit=true`);
  };
              ...
              <div className="flex-1 p-3 rounded-md border bg-background shadow-sm flex items-center justify-between">
                ...
                <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <Button
                    ...
                    variant="ghost"
                    className="h-7 w-7"
                    onClick={() => handleEditStep(step.href)}
                  >
      ...
      <Dialog open={isAddStepDialogOpen} onOpenChange={setIsAddStepDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Masukkan nama untuk langkah atau template surat baru di alur kerja
              ini.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Apakah Anda yakin ingin menghapus alur kerja "{workflow.title}"?
