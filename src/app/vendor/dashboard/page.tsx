...
export default function VendorDashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const doneOrders = vendorSurat.filter((s) =>
      ["Selesai", "Diarsipkan"].includes(s.status),
    ).length;
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Dashboard Vendor</h1>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <StatCard
          title="Total Pesanan Diterima"
          value={stats.total.toString()}
          icon={FileStack}
          description="Jumlah semua pesanan yang pernah dikirim."
        />
        <StatCard
          title="Pesanan Baru / Diproses"
        ...
        <StatCard
          ...
          icon={CheckCircle}
          description="Pesanan yang telah selesai dan diarsipkan."
        />
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Berikut adalah daftar surat pesanan yang telah dikirimkan kepada
            Anda.
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <TableRow key={s.nomor}>
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          statusVariant[
                            s.status as keyof typeof statusVariant
                          ] || "default"
                        }
                      >
                        {s.status}
                    ...
                    <TableCell className="text-right">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          router.push(`/cetak-bundle?nomor=${s.nomor}`)
                        }
                      >
