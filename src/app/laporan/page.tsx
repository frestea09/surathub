...
export default function LaporanPage() {
  ...
  const {
    ...
    const roleUnitMapping: { [key: string]: string } = {
      Direktur: "All",
      "Administrator Sistem": "All",
      "Pejabat Pembuat Komitmen": "Pengadaan",
      "Pejabat Pengadaan Barang Jasa": "Pengadaan",
      "Kepala Bagian Keuangan": "Keuangan",
      "Kepala Bidang Pelayanan Medik": "Pelayanan",
      "Tim Kerja Bidang Umum & Kepegawaian": "Umum",
    };
    ...
    const cards = [
      {
        ...
        value: dataByDate
          .filter((s) => s.jenis === "Surat Keluar")
          .length.toString(),
        description: "Total surat yang dibuat secara internal.",
        icon: Send,
      },
      {
        title: "Total Surat Masuk",
        value: dataByDate
          .filter((s) => s.jenis === "Surat Masuk")
          .length.toString(),
        description: "Total surat yang diterima dari luar.",
        icon: Mailbox,
      },
      {
        title: "Surat Selesai",
        value: dataByDate
          .filter((s) =>
            ["Selesai", "Diarsipkan", "Disetujui"].includes(s.status),
          )
          .length.toString(),
        description: "Total surat yang prosesnya sudah rampung.",
        icon: CheckCircle,
      },
      {
        title: "Surat Ditolak / Revisi",
        value: dataByDate
          .filter(
            (s) => s.status === "Ditolak" || s.status === "Revisi Diminta",
          )
          .length.toString(),
        description: "Total surat yang ditolak atau butuh revisi.",
        icon: XCircle,
      },
    ];
    ...
    const volumeData = monthNames.map((monthName, index) => ({
      name: monthName,
      total: dataByDate.filter((s) => new Date(s.tanggal).getMonth() === index)
        .length,
    }));
    ...
    const statusData = [
      {
        name: "Proses",
        value: dataByDate.filter((s) =>
          ["Draft", "Baru", "Didisposisikan", "Terkirim"].includes(s.status),
        ).length,
      },
      {
        name: "Selesai",
    ...
    return {
      ...
      dynamicStatCards: cards,
      suratVolumeData: volumeData,
  ...
  const handleExport = () => {
    if (filteredData.length === 0) {
      toast({
        ...
        title: "Gagal Ekspor",
        description:
          "Tidak ada data untuk diekspor pada rentang tanggal yang dipilih.",
      });
      return;
    }
    const headers = [
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute(
        "download",
        `laporan-surat-${new Date().toISOString().split("T")[0]}.csv`,
      );
  ...
  const columns: ColumnDef<Surat>[] = [
    ...
    { accessorKey: "judul", header: "Judul" },
    {
      ...
      cell: ({ row }) => (
        <Badge
          variant={
            row.original.jenis === "Surat Masuk" ? "secondary" : "outline"
          }
        >
          {row.original.jenis}
        </Badge>
    ...
    { accessorKey: "dariKe", header: "Dari/Ke" },
    {
      ...
      cell: ({ row }) => (
        <Badge
          variant={
            statusVariant[row.original.status as keyof typeof statusVariant]
          }
        >
          {row.original.status}
        </Badge>
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem
              onClick={() => handleActionClick(row.original, "detail")}
            >
              <FileSearch className="mr-2 h-4 w-4" />
              Lihat Detail Surat
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => handleActionClick(row.original, "alur")}
            >
              <FileSearch className="mr-2 h-4 w-4" />
              Lihat Alur Lengkap
            </DropdownMenuItem>
    ...
    <AppLayout>
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold md:text-2xl">
          Laporan {viewAsUser ? `(${viewAsUser.jabatan})` : ""}
        </h1>
        <div className="w-64">
          ...
          <>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Jumlah total surat yang tercatat setiap bulan dalam rentang
                  waktu yang dipilih.
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Proporsi surat berdasarkan statusnya saat ini.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Data Rinci Surat</CardTitle>
            <CardDescription>
              Tabel data semua surat berdasarkan filter di atas. Gunakan tombol
              di pojok untuk mengunduh data.
            </CardDescription>
          </div>
          ...
          {selectedSurat && (
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  id="tanggal"
                  value={formatDate(selectedSurat.tanggal)}
                  readOnly
                  className="col-span-3"
                />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="dariKe" className="text-right">
                  Dari/Ke
                </Label>
                <Input
                  id="dariKe"
                  value={selectedSurat.dariKe}
                  readOnly
                  className="col-span-3"
                />
              </div>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  id="penanggungJawab"
                  value={selectedSurat.penanggungJawab}
                  readOnly
                  className="col-span-3"
                />
              </div>
      ...
      <Dialog open={isAlurOpen} onOpenChange={setIsAlurOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Linimasa perjalanan surat nomor{" "}
              <span className="font-semibold">{selectedSurat?.nomor}</span>.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
                ...
                <li className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Oleh: Admin, Pada: {formatDate(selectedSurat.tanggal)}
                    </p>
                    <p className="text-sm">
                      Surat diterima dari {selectedSurat.dariKe}.
                    </p>
                  </div>
                </li>
                <li className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Oleh: Admin, Kepada: {selectedSurat.penanggungJawab}
                    </p>
                    <p className="text-sm">
                      Surat diteruskan untuk ditindaklanjuti.
                    </p>
                  </div>
                </li>
                <li className="flex items-start">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Oleh: {selectedSurat.penanggungJawab}
                    </p>
                    <p className="text-sm">
                      {["Selesai", "Diarsipkan", "Disetujui"].includes(
                        selectedSurat.status,
                      )
                        ? "Tindakan yang diperlukan telah selesai."
              ...
              <ul className="space-y-4">
                <li className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Oleh: {selectedSurat?.penanggungJawab}, Pada:{" "}
                      {selectedSurat ? formatDate(selectedSurat.tanggal) : ""}
                    </p>
                    <p className="text-sm">
                ...
                <li className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      Persetujuan internal untuk pengiriman.
                    </p>
                  </div>
                </li>
                <li className="flex items-start">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {selectedSurat?.status === "Terkirim"
                        ? `Kepada: ${selectedSurat?.dariKe}`
                        : "Surat belum dikirim"}
                    </p>
