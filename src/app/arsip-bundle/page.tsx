...
import { roundHalfUp, formatDate } from "@/lib/utils";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const forwardLinks: {
  [key: string]: { nextType: string[]; refKey: string; sourceKey: string };
} = {
  SPP: { nextType: ["SP"], refKey: "nomorSuratReferensi", sourceKey: "nomor" },
  SP: {
    nextType: ["SP-Vendor"],
    refKey: "nomorSuratReferensi",
    sourceKey: "nomor",
  },
  "SP-Vendor": {
    nextType: ["BA"],
...
const calculateTotal = (surat: Surat): number => {
  ...
  const subtotal = items.reduce(
    (sum: number, item: any) =>
      sum + (item.volume || item.jumlah) * item.hargaSatuan,
    0,
  );
  const totalDiskon = items.reduce(
    (sum: number, item: any) =>
      sum +
      (item.volume || item.jumlah) *
        item.hargaSatuan *
        ((item.diskon || 0) / 100),
    0,
  );
...
export default function ArsipBundlePage() {
  ...
  const allBundles = useMemo(() => {
    ...
    const findChildrenRecursive = (doc: Surat, chain: Surat[]) => {
      ...
      if (link) {
        const children = surat.filter(
          (s) =>
            link.nextType.includes(s.tipe) &&
            (s.data.formData?.[link.refKey] || s.data[link.refKey]) ===
              (doc.data.formData?.[link.sourceKey] || doc.data[link.sourceKey]),
        );
    ...
    const bundleHeads = surat.filter(
      (s) => s.tipe === "SPP" || s.tipe === "SPU",
    );
        ...
        documents.some(
          (doc: Surat) =>
            ...
            doc.nomor
              .toLowerCase()
              .split(/[\/\.-]/)
              .some((part) => part.includes(lowerCaseSearchTerm)),
        );

      const matchesDateRange =
        !date?.from ||
        documents.some((doc: Surat) => {
  ...
  const filteredBundles = useMemo(
    () => filterItems(allBundles, true),
    [allBundles, searchTerm, date],
  );
  const filteredVendorOrders = useMemo(
    () => filterItems(vendorOrders, false),
    [vendorOrders, searchTerm, date],
  );
  ...
  const handleDeleteConfirm = () => {
    ...
    toast({
      title: "Draf Dihapus",
      description: `Draf surat nomor ${suratToDelete.nomor} telah dihapus.`,
    });
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            variant={
              statusVariant[
                row.original.status as keyof typeof statusVariant
              ] || "secondary"
            }
          >
            {row.original.status}
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button asChild variant="outline" size="sm">
            <Link
              href={`/cetak-bundle?nomor=${row.original.nomor}&tipe=${row.original.tipe}`}
            >
    ...
    <AppLayout>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Cari berdasarkan nomor surat, perihal, nama vendor, atau bagian no.
            surat (misal: 06-FAR, ppkrsud, 2025).
          </CardDescription>
        </CardHeader>
          ...
          <div className="mt-6">
            {paginatedBundles.length > 0 ? (
                ...
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                  {paginatedBundles.map((bundle) => {
                    ...
                    const lastDate = new Date(
                      bundle[bundle.length - 1].tanggal,
                    );
                    const dateRange =
                      format(firstDate, "dd MMM", { locale: id }) +
                      (firstDate.getTime() !== lastDate.getTime()
                        ? ` - ${format(lastDate, "dd MMM yyyy", { locale: id })}`
                        : ` ${format(lastDate, "yyyy", { locale: id })}`);
                      ...
                      <Card key={head.nomor} className="flex flex-col">
                        <CardHeader>
                          <div className="flex items-start gap-4">
                            ...
                            <div>
                              ...
                              <CardDescription>
                                {dateRange} • {bundle.length} Dokumen
                              </CardDescription>
                            </div>
                          </div>
                        </CardHeader>
                        <CardContent className="flex-grow">
                          <ul className="space-y-1">
                            {bundle.map((doc) => (
                              <li
                                key={doc.nomor}
                                className="flex items-center justify-between text-sm group hover:bg-muted/50 rounded-md p-1 -m-1"
                              >
                                <div className="flex items-center gap-2">
                                  <FileText className="h-4 w-4 text-muted-foreground" />
                                  <span className="text-muted-foreground">
                                    {tipeToLabel[doc.tipe] || doc.tipe}
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <Badge
                                  ...
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent>
                                      <DropdownMenuItem
                                        ...
                                        disabled={
                                          doc.status !== "Draft" &&
                                          doc.status !== "Revisi Diminta"
                                        }
                                      >
                                        <Pencil className="mr-2 h-4 w-4" /> Ubah
                                      </DropdownMenuItem>
                                      <DropdownMenuItem
                                        onClick={() => setSuratToDelete(doc)}
                        ...
                        <div className="p-6 pt-0">
                          <Button asChild className="w-full">
                            <Link
                              href={`/cetak-bundle?nomor=${head.nomor}&tipe=${head.tipe}`}
                            >
                              Lihat & Cetak Bundle
              ...
              <Alert className="mt-6">
                ...
                <AlertDescription>
                  Tidak ada bundle dokumen yang cocok dengan kriteria filter
                  Anda. Coba sesuaikan pencarian atau rentang tanggal Anda.
                </AlertDescription>
              </Alert>
            )}
          </div>
        ...
        <TabsContent value="vendor">
          <Card className="mt-6">
            <CardHeader>
              ...
              <CardDescription>
                Tabel ini berisi daftar semua surat pesanan yang dikirimkan ke
                vendor. Klik tombol "Lihat Bundle" untuk melihat seluruh dokumen
                terkait.
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      <AlertDialog
        open={!!suratToDelete}
        onOpenChange={(open) => !open && setSuratToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Konfirmasi Hapus Draf</AlertDialogTitle>
            <AlertDialogDescription>
              Apakah Anda yakin ingin menghapus draf surat nomor{" "}
              <span className="font-bold">{suratToDelete?.nomor}</span>?
